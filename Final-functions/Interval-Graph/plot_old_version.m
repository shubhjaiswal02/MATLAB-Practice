% Tried to make compatible with MATLAB 7.9.0 
% - xlsx not supported
% - Modified the callback function for the reset button to accept the axes handle as an input argument. This is because nested functions with implicit access to parent workspace variables are not supported in MATLAB 7.9.0.

function plot_old_version()

    % Load Excel data
    [file, path] = uigetfile('*.xls', 'Select Excel file');
    if isequal(file, 0)
    ...
    set(btnReset, 'Callback', @(~,~) resetPlot(ax));
