...
if isfield(handles, 'loaded_data')
    ...
    if ~isempty(selected_columns)
        ...
        % Plot the selected columns
        plot(handles.axes1, selected_data);
        
        % You can customize the plot further if needed, e.g., add labels, title, etc.
        xlabel(handles.axes1, 'X-axis label');
        ylabel(handles.axes1, 'Y-axis label');
        title(handles.axes1, 'Plot of Selected Data from Excel File');
    end
end
